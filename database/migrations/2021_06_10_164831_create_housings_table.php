...
class CreateHousingsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('housings', function (Blueprint $table) {
            ...
            $table->tinyInteger('house_type')->unsigned()->nullable()->comment('户型');
            $table->tinyInteger('toward')->unsigned()->nullable()->comment('朝向');
            $table->string('province')->comment('省');
            $table->string('city')->comment('市');
            $table->string('district')->nullable()->comment('区');
            $table->string('address')->comment('详细地址');
            $table->tinyInteger('heating')->unsigned()->nullable()->comment('供暖方式');
